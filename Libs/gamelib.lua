...
function game.isTeamFullyHealed()
	for pokemonId=1, getTeamSize(), 1 do
		if getPokemonHealthPercent(pokemonId) < 100
			or ( not isPokemonUsable(pokemonId) and ( getTeamSize() == 6 and getPokemonName(6) ~= "Ditto" ) or ( getTeamSize() >= 1 and hasMove(1,"Surf") and getRemainingPowerPoints(1,"Surf") <= 14 ) )  or ( getTeamSize() >= 1 and getPokemonName(1) ~= "Magikarp" and getPokemonLevel(1) < 15 and hasPokemonInTeam("Magikarp") ) then
			return false
		end
	end
	return true
